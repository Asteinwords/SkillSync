...
const PastRoomCard = ({ session, index, myId }) => {
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col py-2 px-3 sm:py-3 sm:px-4 rounded-xl mb-3 transition-all duration-300 shadow-md bg-white/90 backdrop-blur-sm border-l-4 border-blue-300/50 ${
        index === 0 ? 'bg-white/90' :
        index === 1 ? 'bg-white/90' :
        'bg-white/90'
      }`}
    ...
    >
      <div className="flex items-center justify-between">
        <p className="font-semibold text-blue-700 flex items-center gap-2 text-xs sm:text-sm">
          <span className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold">
...
const MainHome = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-blue-200 via-purple-100 to-pink-200 text-gray-900 overflow-hidden p-4 sm:p-6">
      ...
      >
        ...
        >
          <Link to={`/users/${myId}/profile`}>
            <motion.img
              ...
              alt="Profile"
              className="w-20 sm:w-24 h-20 sm:h-24 rounded-full border-4 border-blue-300/50 shadow-md object-cover cursor-pointer"
              whileHover={{ scale: 1.2, boxShadow: '0 0 25px rgba(59, 130, 246, 0.5)' }}
        ...
        <motion.h2
          variants={itemVariants}
          className="text-3xl sm:text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-pink-500"
        >
          Welcome, {user.name}
        </motion.h2>
        <motion.h1
          variants={itemVariants}
          className="text-xl sm:text-2xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-pink-500 hidden sm:block"
        >
      ...
      <motion.div
        ...
        className="bg-white/90 backdrop-blur-xl rounded-2xl p-4 border-2 border-blue-300/50 shadow-2xl transform hover:scale-105 transition-transform duration-300 w-full mx-auto max-w-4xl mt-6"
      >
        <h2 className="text-2xl sm:text-3xl font-extrabold text-center text-blue-700 mb-3">Top 3 Leaders</h2>
        {users.length === 0 ? (
          <motion.p className="text-center text-gray-600 text-sm sm:text-base" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            No users found.
          </motion.p>
        ) : (
          <div className="space-y-4">
            {users.map((user, index) => (
              <motion.div
                key={user._id}
                variants={itemVariants}
                className={`group relative flex flex-col sm:flex-row items-center justify-between py-2 px-3 rounded-xl bg-white/90 backdrop-blur-sm border-l-4 ${
                  index === 0 ? 'border-yellow-400' : index === 1 ? 'border-gray-400' : 'border-orange-400'
                }`}
                whileHover={{ scale: 1.05, boxShadow: '0 0 15px rgba(59, 130, 246, 0.5)', zIndex: 10 }}
                style={{ overflow: 'visible' }}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="truncate">
                    <span className="font-semibold text-blue-600 text-sm sm:text-base">{user.name}</span>
                    <p className="text-xs text-gray-600 truncate">{user.email}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-blue-600 text-sm sm:text-base">{user.points} pts</p>
                  <span className={`inline-block text-xs sm:text-sm px-2 py-1 rounded-full ${badgeStyles[user.badge] || 'bg-gray-100 text-gray-600'}`}>
                    {user.badge}
                  </span>
                </div>
              </motion.div>
      ...
      <div className="flex flex-col sm:flex-row gap-6 mt-6">
        <motion.div
          variants={cardVariants}
          className="bg-white/90 backdrop-blur-xl rounded-2xl p-4 border-2 border-blue-300/50 shadow-2xl transform hover:scale-105 transition-transform duration-300 w-full sm:w-1/2 mx-auto max-w-md"
          style={{ overflow: 'visible' }}
        >
          <h2 className="text-2xl sm:text-3xl font-extrabold text-center text-blue-700 mb-3">Recent Sessions</h2>
        ...
        <motion.div
          ...
          style={{ overflow: 'visible' }}
        >
          <h2 className="text-2xl sm:text-3xl font-extrabold text-center text-blue-700 mb-3">Quick Tips</h2>
          <AnimatePresence>
            <div className="space-y-4">
              {currentTips.map((tip, index) => (
                <motion.div
                  ...
                  animate="visible"
                  exit="exit"
                  className="bg-gray-100/50 rounded-xl p-3 shadow-md"
                  style={{ overflow: 'visible' }}
                >
                  <p className="text-sm sm:text-base text-gray-800">{tip}</p>
                </motion.div>
              ))}
            </div>
          </AnimatePresence>
      ...
      <motion.div
        variants={cardVariants}
        className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 border-2 border-blue-300/50 shadow-2xl transform hover:scale-110 transition-transform duration-300 w-full mx-auto max-w-4xl mt-6"
      >
        <h2 className="text-3xl sm:text-4xl font-extrabold text-center text-blue-700 mb-4">Community Highlights</h2>
        <motion.div
          variants={cardVariants}
          className="bg-gray-100/50 rounded-2xl p-4"
        >
          <div className="flex flex-col sm:flex-row justify-around items-center gap-4">
            <div className="text-center">
              <p className="text-lg sm:text-xl text-gray-600">Total Posts</p>
              <p className="text-2xl sm:text-3xl font-bold text-blue-600">{communityStats.totalPosts}</p>
            </div>
            <div className="text-center">
              <p className="text-lg sm:text-xl text-gray-600">Active Users Today</p>
              <p className="text-2xl sm:text-3xl font-bold text-blue-600">{communityStats.activeUsersToday}</p>
            </div>
          </div>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        ...
        className="bg-white/90 backdrop-blur-xl rounded-2xl p-4 border-2 border-blue-300/50 shadow-2xl transform hover:scale-105 transition-transform duration-300 w-full mx-auto max-w-4xl mt-6"
      >
        <h2 className="text-2xl sm:text-3xl font-extrabold text-center text-blue-700 mb-3">What People Say About You</h2>
        {feedbacks.length === 0 ? (
          <motion.p
          ...
          <div className="space-y-4">
            {feedbacks.map((fb, index) => (
              <motion.div
                key={index}
                variants={itemVariants}
                className="flex items-center gap-3 py-2 px-3 rounded-xl bg-gray-100/50"
                style={{ overflow: 'visible' }}
              >
